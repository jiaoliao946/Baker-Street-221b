    ...
    - 可以使用Markdown Admonitions，共有NOTE、TIP、IMPORTANT（本站将其作为参考文档列写处）、WARNING和CAUTION，与Typora中显示的颜色一致。
        ...
        - 进阶用法
            ...
            - 更改颜色
                ...
                ​	由于实现方式是修改CSS元素的类名（后续可能会更改），所以颜色只能为有限几个，需添加配置`className:`，其后填入`note`、`tip`、`undefined`、`warning`或`attention`，分别对应蓝、绿、紫、棕和红。
...
4. 内容要求：
    ...
    - 如果某篇文章的适用人群和硬件环境与所属合集或系列不同，需额外说明；
    - 每个段落开头如需缩进，统一用四个半角（即两个全角）空格；
    - 若使用的某个符号有全角形式和半角形式，东亚语言文章采用全角形式，其它语言文章采用半角形式；
    - 指导读者以文本形式输入的内容及各种带有指导意味的强调或特指内容用一对`` ` ``包含；
    - 键盘按键用`<kbd>`和`</kbd>`包含；
    - 文章末尾通过Markdown Admonitions IMPORTANT列出该文章的参考文档链接。
